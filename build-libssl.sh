...
#
#  Modified by Travis B. Meisenheimer on November 7, 2014
#
...
if [ ! -e openssl-${VERSION}.tar.gz.md5 ]; then
	echo "Downloading openssl-${VERSION}.tar.gz.md5"
    curl -O http://www.openssl.org/source/openssl-${VERSION}.tar.gz.md5
else
	echo "Using openssl-${VERSION}.tar.gz.md5"
fi

echo "Validating authenticity"
ACTUAL_MD5=`md5 -q  openssl-${VERSION}.tar.gz`
EXPECTED_MD5=`cut -d* -f1  openssl-${VERSION}.tar.gz.md5`

if [ ${ACTUAL_MD5} == ${EXPECTED_MD5} ]; then
  echo "MD5 values matched."
else
  echo "MD5 values did not match, aborting."
  exit 1;
fi
